/*
    epd.c - source file for the gdew042t2 4.2 inch e-paper display
...
#define LO 0
#define HI 1
#define EPD_DC_CMD  LO
#define EPD_DC_DATA HI
#define EPD_RST_LO  LO
...
void select (void)
{
    // wait until SSI0 not busy/transmit FIFO empty
    while(LL_SPI_GetTxFIFOLevel(EPD_SPI) != LL_SPI_TX_FIFO_EMPTY);
    // writepin(EPD_Port, EPD_CS_PIN, EPD_CS_LO);
}
...
static inline void
wait_until_idle (void)
{
    while (readpin(EPD_Port, EPD_BSY_Pin) == EPD_BSY_LO);
}
...
spi_bus_config_t epd_bus_cfg = {
    .miso_io_num = -1,
    .mosi_io_num = EPD_MOSI_Pin,
    .sclk_io_num = EPD_CLK_Pin,
    .quadwp_io_num = -1,
    .quadhd_io_num = -1,
    .max_transfer_sz = PARALLEL_LINES * 8,
};
spi_device_interface_config_t epd_dev_cfg = {
...
void esp32_init(void)
{
    ...
    ret = spi_bus_initialize(EPD_HOST, &epd_bus_cfg, DMA_CHAN);
    ESP_ERROR_CHECK(ret);

    ret = spi_bus_add_device(EPD_HOST, &epd_dev_cfg, &epd_spi);
    ESP_ERROR_CHECK(ret);

    ret = gpio_set_direction(EPD_DC_Pin, GPIO_MODE_OUTPUT);
    ESP_ERROR_CHECK(ret);

    ret = gpio_set_direction(EPD_RST_Pin, GPIO_MODE_INPUT_OUTPUT);
    ESP_ERROR_CHECK(ret);

    ret = gpio_set_direction(EPD_BSY_Pin, GPIO_MODE_INPUT);
    ESP_ERROR_CHECK(ret);
}
...
{
    esp_err_t ret;
    spi_cmd.tx_buffer = &cmd;
    ret = spi_device_polling_transmit(epd_spi, &spi_cmd);
    ESP_ERROR_CHECK(ret);
}
...
{
    esp_err_t ret;
    spi_data.tx_buffer = &data;
    ret = spi_device_polling_transmit(epd_spi, &spi_data);
    ESP_ERROR_CHECK(ret);
}

void select (void)
{
    // gpio_set_level(EPD_CS_Pin, LO);
}

void deselect (void)
{
    // gpio_set_level(EPD_CS_Pin, HI);
}

bool
epd_is_idle(void)
{
    return (gpio_get_level(EPD_BSY_Pin) == EPD_BSY_HI);
}

static inline void
wait_until_idle (void)
{
    while (gpio_get_level(EPD_BSY_Pin) == EPD_BSY_LO) {
        delayms(10);
    }
}
...
{
    esp_err_t ret;
    ret = gpio_set_level(EPD_RST_Pin, EPD_RST_LO);
    ESP_ERROR_CHECK(ret);
    delayms(100);

    ret = gpio_set_level(EPD_RST_Pin, EPD_RST_HI);
    ESP_ERROR_CHECK(ret);
    delayms(100);
}
// ESP32
